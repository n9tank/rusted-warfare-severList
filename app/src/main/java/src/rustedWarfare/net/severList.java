...
public class severList {
 ...
 room room= new room();
  ...
  room.ip = list[4];
  room.severName = list[7].trim();
// room.passage=list[8].equals("true");
  String map=list[9].trim();
  int len=0; 
  if(map.endsWith(".tmx"))len=4;
  if(map.endsWith(".rwsave"))len=7;
  room.map = map.substring(map.lastIndexOf('/')+1,map.length()-len);
